...
export const MessageFormatter = {
    ...
    async format(text) {
        ...
        // Step 1: Extract code/HTML/SVG blocks and replace with placeholders.
        ...
        // Step 3: Asynchronously re-insert the rendered blocks.
        html = await this._reinsertBlocks(html, blocks);

        // Step 4: Process custom image generation tags.
        html = this._processImageTags(html);

        // Step 5: Intelligently wrap remaining text in <p> tags.
    ...
    /**
     * Renders a standard code block.
     * @private
     */
    _renderCodeBlock(block) {
        const escapedContent = block.content.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        const lang = block.lang.toLowerCase();
        return `<div class="code-block-wrapper is-collapsible is-collapsed"><div class="code-block-header"><span>${lang}</span><div class="code-block-actions"></div></div><div class="collapsible-content"><pre><code class="language-${lang}">${escapedContent}</code></pre></div></div>`;
    },
    
    /**
     * Renders an HTML preview block, inlining any external scripts.
     * @private
     */
    ...
    /**
     * Fetches external scripts and embeds them as inline scripts.
     * @private
     */
